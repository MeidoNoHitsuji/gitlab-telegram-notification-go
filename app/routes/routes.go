...
func New(Secret string) *mux.Router {
	...
	router.Handle("/webhook/toggle/{user_id}", tollbooth.LimitFuncHandler(tollbooth.NewLimiter(1, &limiter.ExpirableOptions{DefaultExpirationTTL: time.Second * 10}), GetWebToggle)).Methods("GET")
